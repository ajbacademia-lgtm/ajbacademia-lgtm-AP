...
import { VisitorActivity, VisitorAnalyticsSummary, CookieSettings, VisitorActivityAction } from '../../../types';
import { DEFAULT_COOKIE_SETTINGS } from '../../services/visitorActivity.service';
...
export const CookieVisitorDashboard: React.FC = () => {
  ...
  // Simulate a live test visitor event
  const handleSimulateVisitorEvent = async () => {
    const mockPages = [
      { action: 'PAGE_VIEW' as const, path: '/journals', title: 'Browse Academic Journals' },
      { action: 'JOURNAL_VIEW' as const, path: '/journal/j1', title: 'International Journal of Agricultural Systems', details: { journalId: 'j1', journalTitle: 'International Journal of Agricultural Systems' } },
      { action: 'ARTICLE_VIEW' as const, path: '/article/a1', title: 'Optimizing Precision Irrigation Through IoT', details: { articleId: 'a1', articleTitle: 'Optimizing Precision Irrigation Through IoT', journalId: 'j1' } },
      { action: 'DOWNLOAD_PDF' as const, path: '/article/a1/pdf', title: 'Downloaded Full Manuscript PDF: Precision Irrigation', details: { articleId: 'a1', articleTitle: 'Optimizing Precision Irrigation Through IoT' } },
      { action: 'SEARCH' as const, path: '/search?q=machine+learning', title: 'Searched "machine learning"', details: { query: 'machine learning', resultsCount: 28 } },
      { action: 'COPY_CITATION' as const, path: '/article/a1/cite', title: 'Copied APA 7th Citation', details: { format: 'APA 7th', articleId: 'a1' } },
      { action: 'CONSENT_UPDATE' as const, path: '/cookies', title: 'Visitor Accepted All Cookie Categories', details: { acceptedAll: true, analytics: true, functional: true, marketing: true } }
    ];

    const randomMock = mockPages[Math.floor(Math.random() * mockPages.length)];
    const mockCountries = ['United States', 'Germany', 'United Kingdom', 'Japan', 'Canada', 'Australia', 'Brazil', 'South Korea'];
    const mockBrowsers = ['Chrome', 'Safari', 'Firefox', 'Edge'];
    const mockDevices: ('desktop' | 'mobile' | 'tablet')[] = ['desktop', 'desktop', 'mobile', 'tablet'];

    const testPayload: Partial<VisitorActivity> = {
      visitorId: `vis_sim_${Math.floor(1000 + Math.random() * 9000)}`,
      sessionId: `sess_sim_${Math.floor(1000 + Math.random() * 9000)}`,
      actionType: randomMock.action,
      pagePath: randomMock.path,
      pageTitle: randomMock.title,
      details: randomMock.details || {},
      device: mockDevices[Math.floor(Math.random() * mockDevices.length)],
      browser: mockBrowsers[Math.floor(Math.random() * mockBrowsers.length)],
      country: mockCountries[Math.floor(Math.random() * mockCountries.length)],
      timestamp: new Date().toISOString()
    };

    try {
      await safeFetchJson('/api/visitor-activities', {
        method: 'POST',
        body: JSON.stringify(testPayload)
      });
      showToast(`Simulated live event: [${testPayload.actionType}] from ${testPayload.country}`, 'info');
      fetchData(true);
    } catch (err: any) {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              className="px-3.5 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded text-xs font-semibold flex items-center gap-1.5 shadow-xs transition-all"
              title="Simulate a live visitor action"
            >
              <Play size={12} />
                      ...
                      <tr>
                        <td colSpan={7} className="p-12 text-center text-gray-400">
                          ...
                          <button
                            type="button"
                            onClick={handleSimulateVisitorEvent}
                            className="mt-3 px-4 py-2 bg-indigo-600 text-white rounded text-xs font-semibold hover:bg-indigo-700"
                          >
